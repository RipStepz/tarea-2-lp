...
void inicializarTablero(int tamano) {
    ...
    for (int i = 0; i < tamano; i++) {
        ...
        for (int j = 0; j < tamano; j++) {
            // Para cada celda, reservamos memoria para un espacio en blanco y casteamos a void*
            tablero[i][j] = (void *)malloc(2 * sizeof(char));  // " " más el terminador '\0'
            strcpy((char *)tablero[i][j], " ");  // Guardamos solo un espacio en blanco
        }
    }
}
...
void modificarCelda(int x, int y, const char *nuevoContenido) {
    ...
    if (fila >= 0 && fila < tamano_global && columna >= 0 && columna < tamano_global) {
        ...
        strcpy((char *)tablero[fila][columna], nuevoContenido);
    } else {
        printf("Índices fuera de los límites del tablero, letra en conflicto : %s, Coordenadas (%d , %d) \n", nuevoContenido , x , y);
    }
}
